 ...
 *
 * @author 	ThemeJR
 * @package matjar/template-parts/global
 * @since 1.0
 */
...
if( 'portfolio' == get_post_type() ){
	$pagination_style		= matjar_get_loop_prop( 'portfolio-pagination-type' );
	...
	$pagination_style 		= matjar_get_loop_prop( 'blogs-pagination-type' );
...
<nav class="matjar-pagination">
	...
	if( $pagination_style != 'default' ){
		...
		if( get_next_posts_link() ) {?>
			<div class="matjar-ajax-load <?php echo esc_attr($pagination_style); ?>" 
		...
		echo paginate_links(
			apply_filters(
				...
				array( // WPCS: XSS ok.
					...
					'total'     => $total,
					'prev_text' => esc_html__( 'Previous', 'matjar' ),
					'next_text' => esc_html__( 'Next', 'matjar' ),
					'type'      => 'list',
					'end_size'  => 2,
					'mid_size'  => 2,
				)
