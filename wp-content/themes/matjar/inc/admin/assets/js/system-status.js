...
jQuery( function ( $ ) {
	...
	var wcSystemStatus = {
		...
		generateReport: function() {
			...
			$( '#matjar-system-status .themejr-dash-title,#matjar-system-status table tbody' ).each( function() {
				if ( $( this ).hasClass( 'themejr-dash-title' ) ) {
					var label =  $( this ).text();
                    report = report + '\n### ' + $.trim( label ) + ' ###\n\n';
				} else {
			...
			try {
				$( '#matjar-debug-report' ).slideDown();
				$( '#matjar-debug-report' ).find( 'textarea' ).val( '`' + report + '`' ).focus().select();
				$( this ).fadeOut();
				return false;
			} catch ( e ) {
				/* jshint devel: true */
