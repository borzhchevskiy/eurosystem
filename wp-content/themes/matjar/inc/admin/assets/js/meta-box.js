(function($){
    ...
    var MatjarMetabox = {
        initialize: function() {
            MatjarMetabox.meta_box_tab();
            MatjarMetabox.required_field();
            MatjarMetabox.widget_select2_process();
        },
        meta_box_tab: function() {
           
			var tabBoxes 			= jQuery(matjar_meta_data.meta_box_ids),
			...
			metabox_wrap_html 		= '<div class="themejr-meta-tabs-wrap postbox"><div class="postbox-header"><h2 class="hndle ui-sortable-handle">'+ matjar_meta_data.meta_box_title +'</h2><div class="handle-actions hide-if-no-js"><button type="button" class="handlediv" aria-expanded="true"><span class="screen-reader-text">Toggle panel: Matjar Options</span><span class="toggle-indicator" aria-hidden="true"></span></button></div></div><div id="themejr-tabbed-meta-boxes"></div></div>';
			;
			var meta_boxes = matjar_meta_data.meta_box_ids.split(",");
			...
			if(product_data.length > 0){
				product_data.after(metabox_wrap_html);
			}
			else if(visual_composer.length > 0){
				visual_composer.after(metabox_wrap_html);
			}else{
				 jQuery('#normal-sortables').prepend(metabox_wrap_html);
			}
			jQuery(tabBoxes).appendTo('#themejr-tabbed-meta-boxes');
            ...
            var menu_html 		= '<ul id="themejr-meta-box-tabs" class="clearfix">\n';
            var total_hidden 	= 0;
            ...
            jQuery('#themejr-tabbed-meta-boxes').before(menu_html);
            jQuery('#themejr-meta-box-tabs a:first').addClass('active');

            jQuery('.editor-tab0').addClass('active').show();

            jQuery('.themejr-meta-tabs-wrap').on('click', '.handlediv', function() {
            ...
            jQuery('#themejr-meta-box-tabs li').on('click', 'a', function(event) {
				event.preventDefault();
                jQuery(tabBoxes).removeClass('active').hide();
                jQuery('#themejr-meta-box-tabs a').removeClass('active');
        ...
        widget_select2: function(event, widget) {
            if (typeof (widget) == "undefined") {
                $('#widgets-right select.widget-select2:not(.select2-ready)').each(function(){
                    MatjarMetabox.widget_select2_item(this);
                });
            }
            else {
        ...
        widget_select2_process: function() {
            $(document).on('widget-added', MatjarMetabox.widget_select2);
            $(document).on('widget-updated', MatjarMetabox.widget_select2);
            MatjarMetabox.widget_select2();
        }
    };
    $(document).ready(function(){
        MatjarMetabox.initialize();
    });   
