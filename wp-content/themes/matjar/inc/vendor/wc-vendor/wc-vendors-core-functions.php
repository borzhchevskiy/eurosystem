...
add_action( 'init','matjar_wc_hook' );

function matjar_wc_hook(){
	if ( get_option('wcvendors_display_label_sold_by_enable') == 'yes' ) {
		...
		if($sold_by_template == 'theme'){
			...
			add_action( 'matjar_shop_loop_item_title', 'matjar_wc_loop_sold_by_label', 21 );
			add_action( 'woocommerce_single_product_summary', 'matjar_wc_item_sold_by_label',8 ); 
	...
	add_filter( 'body_class', 'matjar_wc_body_class' );
}

function matjar_wc_body_class( $classes ) {
		if ( class_exists( 'WC_Vendors' ) ) {
			...
			if ( is_page( $orders_page_id ) ) {
				$classes[] = 'matjar-wc-vendors';
			} elseif ( $shop_settings_page == get_the_ID() ) {
				$classes[] = 'matjar-wc-vendors wc-vendors-shop-settings';
			} elseif( $shop_dashboard_page == get_the_ID() ){ 
				$classes[] = 'matjar-wc-vendors wc-vendors-dashboard';
			}
		}
...
function matjar_wc_loop_sold_by_label(){
	$sold_by_loop = matjar_get_option( 'enable-sold-by-in-loop' , 1 );
	if( !$sold_by_loop ) { return false; }
	matjar_get_wc_vendor_name();
}

function matjar_wc_item_sold_by_label(){
	$sold_by_single = matjar_get_option( 'enable-sold-by-in-single' , 1 );
	if( !$sold_by_single ) { return false; }
	matjar_get_wc_vendor_name();
}

function matjar_get_wc_vendor_name(){
	...
	$sold_by_label 	= apply_filters('wcvendors_sold_by_in_loop',esc_html__( 'Sold By : ', 'matjar' ));
	?>
