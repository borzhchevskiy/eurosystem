<?php 
if( ! class_exists( 'Matjar_Mega_Menu_Walker' )) {
	class Matjar_Mega_Menu_Walker extends Walker_Nav_Menu {
		/**
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if( $depth == 0 && $enable =='enabled' ) {
				...
				$classes[] = 'matjar-megamenu-item-' . $design;
				$classes[] = 'matjar-' . ( (  in_array( $design, array( 'custom-size', 'full-width' ) ) ) ? 'megamenu-dropdown' : 'simple-dropdown' );
			}
			...
			if( $attachment_id ) {
				$icon_url = matjar_get_image_src($attachment_id,'thumnail');
			}
			...
			if( ($depth == 0 && $enable =='enabled') && !empty($custom_block) && !$args->walker->has_children) {
					if($design == 'custom-size') {	
						...
						$containerWidth		= matjar_get_option( 'theme-container-width', 1200 );
						$style_mg_holder	= 'style = "width:'.($containerWidth-30).'px"';
					}
					$item_output .= "\n$indent<div class=\"matjar-megamenu-wrapper\">\n";
					$item_output .= "\n$indent<div class=\"matjar-megamenu-holder\" $style_mg_holder>\n";
					$item_output .= matjar_block_get_content($custom_block);
					$item_output .= "\n$indent</div>\n";
					$item_output .= "\n$indent</div>\n";
...
if( ! class_exists( 'Matjar_Menu_Walker' )) {
	class Matjar_Menu_Walker extends Walker_Nav_Menu {
		/**
		 * Starts the list before the elements are added.
		 *
		 * @see Walker::start_lvl()
