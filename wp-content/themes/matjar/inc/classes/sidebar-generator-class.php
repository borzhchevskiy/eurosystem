<?php
if (!class_exists('Matjar_sidebar')) {

	class Matjar_sidebar{
		
		public $sidebars  = array();
		public $stored = "matjar_sidebars";
		public $paths  = array();

		function __construct() {
			$this->title	= esc_html__('Add Custom Widget Area', 'matjar');
			$this->stored	= 'matjar_sidebars';
		...
		public function enqueue_assets() {
			...
			wp_enqueue_script('themejr-sidebar-script' , MATJAR_FRAMEWORK_URI . 'admin/assets/js/custom_sidebar.js');
			wp_enqueue_style( 'themejr-sidebar-style' , MATJAR_FRAMEWORK_URI . 'admin/assets/css/custom_sidebar.css');
		}
		...
		public function add_field() {
			...
			$output .= "\n  <h3>". $this->title ."</h3>";
			$output .= "\n    <p><input size='30' type='text' value='' placeholder = '". esc_attr__('Enter Name for new Widget Area', 'matjar') ."' name='form-add-widget' /></p>";
			$output .= "\n    <input class='button button-primary' type='submit' value='". esc_attr__('Add Widget Area', 'matjar') ."' />";
			$output .= "\n    <input type='hidden' name='custom-sidebar-nonce' value='". wp_create_nonce('custom-sidebar-nonce') ."' />";
			$output .= "\n  </form>";
			$output .= "\n</script>\n";
			echo apply_filters( 'matjar_template_add_widget_js', $output ); // WPCS: XSS OK.
		}
	...
	$obj_matjar_sidebar = new Matjar_sidebar();
}