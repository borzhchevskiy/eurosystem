 ...
 *
 * @author 	ThemeJR
 * @package matjar/inc/classes
 * @since 1.0
 */
 
if( ! class_exists( 'Matjar_Breadcrumb' )) {
	class Matjar_Breadcrumb{
		/**
		 * Breadcrumb trail.
		 *
		 * @var array
		...
		public function get_breadcrumb() {
			return apply_filters( 'matjar_get_breadcrumb', $this->crumbs, $this );
		}
		...
		public function generate() {
			...
			if ( ! is_front_page() ) {
				$this->add_crumb(esc_html__('Home', 'matjar'),home_url( '/' ));
			} elseif ( is_home() ) {
		...
		private function add_crumbs_home() {
			$this->add_crumb(esc_html__('Home', 'matjar'));
		}
		...
		private function add_crumbs_tag() {
			...
			$this->add_crumb( sprintf( __( 'Article tagged &ldquo;%s&rdquo;', 'matjar' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
		}
		...
		private function add_crumbs_product_tag() {
			...
			/* translators: %s: tag name */
			$this->add_crumb( sprintf( __( ' Products tagged &ldquo;%s&rdquo;', 'matjar' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
		}
			...
			if(!empty($taxonomy )){
				$terms = wp_get_object_terms(
					$post->ID, $taxonomy, apply_filters(
						'matjar_breadcrumb_product_terms_args', array(
							'orderby' => 'parent',
							'order'   => 'DESC',
						)
					)
				...
				if ( $terms ) {
					$main_term = apply_filters( 'matjar_breadcrumb_main_term', $terms[0], $terms );
		...
		public function get_archive_name($post_type){
			...
			if ($post_type == 'portfolio') {
				$archive_title = esc_html__('Portfolio','matjar');
			} else {
				$post_type_object = get_post_type_object( $post_type );
				if ( is_object( $post_type_object ) ) {
		...
		function add_crumbs_leaf( $object_type = '' ) {
			...
			switch( $object_type ) {
				...
				case 'search':
					...
					$title = sprintf( __( 'Search - %s', 'matjar' ), $search );
					break;
				case '404':
					$title = esc_html__( '404', 'matjar' );
					break;
				case 'bbpress_search':
					$title = sprintf( __( 'Search - %s', 'matjar' ), esc_html( get_query_var( 'bbp_search' ) ) );
					break;
				case 'bbpress_user':
