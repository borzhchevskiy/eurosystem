...
$attachment_ids 	= $product->get_gallery_image_ids(); // Matjar
$wrapper_classes   	= apply_filters(
	...
	array(
		...
		'matjar-product-gallery-' . ( $attachment_ids ? 'with-thumbnails' : 'without-thumbnails' ),
		'images',
	)
);
$is_quick_view      = matjar_get_loop_prop( 'is_quick_view' );
$gallery_layout 	= matjar_get_product_gallery_layout();	
...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>" data-columns="<?php echo esc_attr( $columns ); ?>" >
	<div class="woocommerce-product-gallery__wrapper">
		<?php do_action( 'matjar_product_gallery_top' ); ?>
		<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $single_product_gallery_classes ) ) ); ?>">
		<?php
		if ( $post_thumbnail_id ) {
			array_unshift( $attachment_ids, $post_thumbnail_id );
			$html = matjar_wc_get_gallery_html( $attachment_ids, true );
		} else {
			$html  = '<div class="woocommerce-product-gallery__image--placeholder">';
			$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src( 'woocommerce_single' ) ), esc_html__( 'Awaiting product image', 'matjar' ) );
			$html .= '</div>';
		}
		...
		<div class="single-product-images-btns">
			<?php do_action( 'matjar_product_gallery_bottom' ); ?>
		</div>
	</div>
