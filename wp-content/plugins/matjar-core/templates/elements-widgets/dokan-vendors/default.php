						...
						<div class="matjar-store-content-wrapper <?php echo esc_attr($has_bg_class);?>" style="background-image: url( '<?php echo is_array( $store_banner_url ) ? esc_attr( $store_banner_url[0] ) : esc_attr( $store_banner_url ); ?>');">
							<div class="matjar-store-content-container">
								<?php if ( $is_store_featured ) : ?>
									<div class="matjar-store-featured">							
										<div class="featured-label"><?php esc_html_e( 'Featured', 'matjar-core' ); ?></div>
									</div>
								<?php endif ?>
								<div class="matjar-store-data">
									<h2><?php echo esc_html( $store_name ); ?></h2>
									<?php if ( ! empty( $store_rating['count'] ) ) { ?>
										<div class="star-rating matjar-store-rating" title="<?php echo sprintf( esc_attr__( 'Rated %s out of 5', 'matjar-core' ), esc_attr( $store_rating['rating'] ) ) ?>">
											<span style="width: <?php echo ( esc_attr( ( $store_rating['rating'] / 5 ) ) * 100 - 1 ); ?>%">
												<strong class="rating"><?php echo esc_html( $store_rating['rating'] ); ?></strong> <?php esc_html_e( 'out of 5', 'matjar-core' ); ?>
											</span>
										</div>
									<?php } ?>
									...
									<?php } ?>
									<!--<div><i class="fa fa-cart"></i><?php _e( 'Total products', 'matjar-core' ); echo ': ' . count_user_posts( $vendor_id, $post_type = 'product' ); ?></div>-->
								</div>
							</div>
						</div>
					</div>
