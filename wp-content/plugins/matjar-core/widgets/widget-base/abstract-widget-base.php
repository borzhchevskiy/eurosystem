...
abstract class Matjar_Widget_Base extends WP_Widget {
    ...
    public function __construct() {
        ...
        $this->attrs = apply_filters( 'matjar_widget_base_attr_arr', array(
                'type' => 'text',
                'class' => '',
                'wrapper_class' => '',
                'label' => '',
        ...
        add_action('admin_enqueue_scripts', array($this,'matjar_widget_scripts'));
        /**/
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        foreach ( $this->settings as $key => $setting ) {
             ...
             */
            $instance[ $key ] = apply_filters( 'matjar_widget_settings_sanitize_option', $instance[ $key ], $new_instance, $key, $setting );
        }
    ...
    public function form($instance) {
        ...
        foreach ($this->settings as $key => $setting) {
            ...
            $desc = $separator = '';
            if( $attr['desc'] ) {
				if($attr['allow_esc_html']){
					$desc = '<span style="clear:both;display:block;"><em>'.esc_html($attr['desc']).'</em></span>';
				}else{
					$desc = '<span style="clear:both;display:block;"><em>'.$attr['desc'].'</em></span>';
				}
            ...
            switch ( $setting['type'] ) {
                        ...
                        <div class="<?php echo esc_attr( $attr['class'] ); ?>" style="<?php echo esc_attr( $attr['css'] ); ?>">
                            ...
                            <div class="image-field">
                                ...
                                <p>
                                    ...
                                    <button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e( 'Use Image', 'matjar-core' ); ?>" data-uploader-btn-txt="<?php esc_attr_e( 'Choose an Image', 'matjar-core' ); ?>">
                                        <?php _e( 'Upload/Add image', 'matjar-core' ); ?>
                                    </button>
                                    <button type="button" class="remove_image_button button" style="<?php echo esc_attr($remove_button_style);?>"><?php _e( 'Remove image', 'matjar-core' ); ?></button>
                                </p>
                            </div>
                        </div>
                        <?php
				...
				case 'warning':
                    ?>
                    <p class="matjar-message-warning">
                        <?php echo wp_kses_data( $attr['label'] ); ?>                        
                    </p>
                    <?php
                    break;
                ...
                default:
                    do_action( 'matjar_widget_field_' . $setting['type'], $key, $value, $setting, $instance );
                    break;
            }
        }
    }
     ...
     * @return void
     */
    public function matjar_widget_scripts($hook){
        if ('widgets.php' === $hook) {
            wp_enqueue_style( 'wp-color-picker' );
            wp_enqueue_media();
            wp_enqueue_script('matjar_widgets_js', MATJAR_CORE_URL . '/widgets/widget-base/js/widgets.js', array('jquery','wp-color-picker'), '1.0.0', true);
        }
    }
